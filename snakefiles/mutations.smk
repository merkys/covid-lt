...
rule dssp_part:
    input:
        "{pdbid}.pdb"
    output:
        "dssp/{pdbid}_{mutation}_{partner1}_{partner2}.part"
    singularity:
        "containers/dssp.sif"
    shell:
        """
        ...
        bin/pdb_select --chain $CHAIN {input} \
                | grep -e ^HEADER -e ^ATOM \
                | dssp --output-format dssp /dev/stdin \
                | grep -vP '\.$' \
                | awk '{{ if( $2 == '$POS' && $3 == "'$CHAIN'" ) {{ print }} }}' \
...
rule mutation_model:
    input:
        "train-dataset-{method}.tab"
    output:
        "binding-evaluator-model-{method}.RData"
    log:
        "binding-evaluator-model-{method}.log"
    singularity:
